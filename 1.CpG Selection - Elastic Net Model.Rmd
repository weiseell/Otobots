...
```{r}
## Data prep ####
# subset to just wild data
sexdata <- sexdata %>% 
  filter(DataType == "Wild" & Age < 16)

alldat <- left_join(sexdata,allCpGs1,by = "indivname")

#remove CpGs with NAs for the model
nas <- colSums(is.na(alldat))
allCpGs1 <- alldat[,which(nas < 1)]
...
modelout <- data.frame(matrix(data = NA,nrow = 100, ncol = 5))
...
for (i in 1:100) {
  ...
  y <- as.matrix(traindat$LogAge)
  x <- as.matrix(traindat[,-1:-11])
...
```{r}
top_CpG_group$ModelType <- "Age"
top_CpG_group_len$ModelType <- "Length"
CpGlist_best_comb <- unique(rbind(top_CpG_group,top_CpG_group_len))

write.csv(CpGlist_best_comb,file = "Output/CpGList_LengthAge_Age4to15_121025.csv",row.names = F)
```
...
tmp <- sexdata[,1:3]
#jpeg(file = "Figures/PerCpGOtolith_SelectedfromAge16to29.jpeg", width = 1200, height = 1000)
