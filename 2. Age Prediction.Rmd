...
load("Input/allCpGs_AddAge1629Samples_nonas.rda")
...
# remove a set of the 20+ individuals for training
testpick <- testpick[1:(nrow(testpick)-2),]
testpick <- testpick[-22,]
...
res_comb_all <- res_comb_all %>% mutate(Age=exp(LogAge),EpiAge=exp(s.0.2869247))
...
res_comb_all_f <- res_comb_all_f %>% mutate(Age=exp(LogAge),EpiAge=exp(s.0.2869247))
...
res_comb_all_m <- res_comb_all_m %>% mutate(Age=exp(LogAge),EpiAge=exp(s.0.2869247))
res_comb_all_m$ModelName <- "Male Only"

res_merged <- rbind(res_comb_all,res_comb_all_f,res_comb_all_m)
jpeg(file = "Figures/AgingClock_Combined_AgeLengthCpGs_AddAge1629.jpeg", width = 900, height = 480)
...
plotdata1 %>% 
  ggplot(aes(x = Length,y = s.0.2869247,color=as.factor(Age),group=as.factor(Age)))+
