...
allCpGs1 <- allCpGs1[,which(!(grepl(pattern = "NC_047158.1",
                                    colnames(allCpGs1),perl = T)))]
allCpGs1_inv <- allCpGs1[,which((grepl(pattern = "NC_047162.1",
                                    colnames(allCpGs1),perl = T)))]
...
#log age data
sexdata1 <- sexdata %>% 
  filter(Age <= 20 & DataType == "Wild" & ObsSex == 2) %>% 
...
modelout <- data.frame(matrix(data = NA,nrow = 100, ncol = 6))
...
for (i in 1:100) {
  ...
  model1 <- cv.glmnet(x,y,family = "gaussian",alpha = 0.5,type.measure = "mae")
...
best_corrsCpG <- data.frame(top_CpG_group,type="Age")
...
best_corrsCpG_len <- data.frame(top_CpG_group,type="Length")
...
```{r}
colnames(best_corrsCpG) <- c("CpGlist_best","n","type")
colnames(best_corrsCpG_len) <- c("CpGlist_best","n","type")

CpGlist_best_comb <- unique(rbind(best_corrsCpG,best_corrsCpG_len))

write.csv(CpGlist_best_comb,file = "Output/CpGList_LengthAge_MAEBestModels_FemaleOnly.csv",row.names = F)
```
