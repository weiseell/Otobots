...
CpGsMeta <- sexdata %>% left_join(allCpGs) %>% filter(Age >= 16)

#remove CpGs with NAs for the model
nas <- colSums(is.na(CpGsMeta))
allCpGs1 <- CpGsMeta[,which(nas < 1)]
...
for (i in 1:100) {
  ...
  x <- as.matrix(traindat[,-1:-9])
  
  model1 <- cv.glmnet(x,y,family = "gaussian",alpha = 0.5,nfolds = 40,grouped = F)
...
write.csv(CpGlist_best_comb,file = "Output/CpGList_LengthAge_addAge1629.csv",row.names = F)
```
