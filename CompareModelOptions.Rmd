...
```{r}
library(mgcv)
gam_model <- gam(Age ~ s(NC_047172.1.26303009),data = testdf)
summary(gam_model)
gam.check(gam_model, k.rep = 1000)
plot(reduceddf$Age,reduceddf$NC_047172.1.26303009)
...
## redo model with correct smooth factors based on edf values
gam_model2 <- gam(Age ~ s(NC_047163.1.29069872) +
                    s(NC_047155.1.30745806) +
                    NC_047159.1.29166056 +
                    s(NC_047172.1.26303009) +
                    NC_047165.1.15408994 +
...
df_preds = data.frame(testdf$Age, predictions2) %>%
  mutate(lower = fit - 1.96 * se.fit,
         upper = fit + 1.96 * se.fit)
