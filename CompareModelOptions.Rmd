...
CpGlist_all <- rbind(CpGlist_best_4to15)
...
names <- cor_age4to19$CpG[which(cor_age4to19$CorAge4to19 > 0.6)]
testdf_cor0.5 <- testdf[,colnames(testdf) %in% names] 
...
reduceddf <- reduceddf[,which(colnames(reduceddf)!="NC_047153.1.32666785" &
                                colnames(reduceddf)!="NC_047151.1.9078" &
                                colnames(reduceddf)!="NC_047163.1.29069872")]
...
### Single variable model test
```{r}
library(mgcv)
CpGs <- as.matrix(testdf %>% dplyr::select(starts_with("NC")))
Ages <- testdf$Age
sigs <- as.data.frame(matrix(data = NA, nrow = ncol(CpGs), ncol = 4))
colnames(sigs) <- c("edf","Ref.df","F","p-value")

for (i in 1:ncol(CpGs)) {
  gam_model <- gam(Ages ~ s(CpGs[,i]),select=T)
  sigs[i,] <- summary(gam_model)[["s.table"]]
  rownames(sigs)[i] <- colnames(CpGs)[i]
}

sigs1 <- sigs %>% 
  filter(`p-value` <= 0.05)

predictions = predict(gam_model,type = 'response',se = TRUE)

df_preds = data.frame(testdf$Age, predictions) %>%
  mutate(lower = fit - 1.96 * se.fit,
         upper = fit + 1.96 * se.fit)

ggplot(aes(x = testdf.Age, y = fit), data = df_preds) +
...
gam_model2 <- gam(Age ~ NC_047151.1.9880 +
                    NC_047151.1.10785 +
                    s(NC_047163.1.29069872) +
                    ...
                    NC_047164.1.2171813 +
                    s(NC_047174.1.21435010) +
                    NC_047174.1.21430655 +
                    s(NC_047172.1.26303009) +
                    s(NC_047172.1.26307361) +
                    NC_047172.1.26308364 +
...
## redo model with correct smooth factors based on edf values
gam_model3 <- gam(Age ~ NC_047153.1.32675405 +
                    s(NC_047172.1.26303009) +
                    NC_047168.1.480431 +
                    NC_047161.1.22201614 +
...
write.csv(preds_comb,file = "PredsComb_BestModels.csv",quote = F,row.names = F, append = F)
```
