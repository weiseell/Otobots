...
testdf1 <- testdf %>% dplyr::select(-indivname,-ObsSex,-Length,-DataType)
model <- lm(Age~.,data = testdf1)

# looking for perfectly colinear variables
ld.vars <- attributes(alias(model)$Complete)$dimnames[[1]]

reduceddf <- testdf1[,which(!(colnames(testdf1) %in% ld.vars))]
...
reduceddf <- reduceddf[,which(colnames(reduceddf)!="NC_047151.1.9078" &
                                colnames(reduceddf)!="NC_047153.1.32666785" &
                                colnames(reduceddf)!="NC_047165.1.15411811")]
